...
export const QuoteForm: React.FC<QuoteFormProps> = ({ onSaveHistory }) => {
  ...
  // Total final = subtotal productos + extras prorrateados + viáticos
  const totalFinal = calculateSubtotal() + totalExtras + viaticos;
  
  // Utilidad = Total venta productos - Costo fábrica (extras se cobran y pagan, se cancelan)
  const totalProfit = calculateSubtotal() - calculateTotalFactoryCost();
  ...
  const generateFinalPDF = async () => {
    ...
    doc.text('HMG!', 20, 20);
    doc.setFontSize(10);
    doc.text('HOME MY GOD!', 20, 26);
    doc.setFontSize(8);
    doc.setFont(undefined, 'normal');
    doc.text('HOME MY GOD QUERETARO', 20, 32);
    
    // Info cotización (derecha)
    doc.setFontSize(14);
    doc.setFont(undefined, 'bold');
    doc.text(`COTIZACIÓN: ${cotizacionNum}`, 130, 15);
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.text(`FECHA: ${fechaHoy}`, 130, 22);
    doc.text(`CLIENTE: ${clientName || '_______________'}`, 130, 30);
    doc.text('VIGENCIA: 15 días', 130, 38);
    ...
    const tableData = selectedItems.map((item, idx) => {
      ...
      const ancho = (item as any).ancho || '-';
      const alto = (item as any).alto || '-';
      const m2 = (item as any).m2 || '-';
    ...
    doc.autoTable({
      startY: 50,
      head: [['No', 'Ubicación', 'Ancho', 'Altura', 'M2', 'Tipo Persiana', 'Tipo Tela', 'Nombre Tela', 'Color', 'Mec.', 'Costo']],
      body: tableData,
      theme: 'grid',
      styles: { 
        fontSize: 7,
    ...
    // --- TOTALES ---
    const subtotal = calculateSubtotal() + totalExtras;
    const descuento = 0; // Puedes agregar campo de descuento después
