...
export const QuoteForm: React.FC<QuoteFormProps> = ({ onSaveHistory }) => {
  ...
  const generateFinalPDF = async () => {
    ...
    // Tabla de productos (precio incluye extras prorrateados)
    const tableData = selectedItems.map(item => {
      ...
      return [item.descripcion, item.cantidad, `$${priceWithExtras.toFixed(2)}`, `$${(priceWithExtras * item.cantidad).toFixed(2)}`];
    });
    ...
    // Total con extras incluidos
    const totalConExtras = calculateSubtotal() + totalExtras;
    
    // Subtotal
    doc.setFontSize(11);
    doc.text(`Subtotal: $${totalConExtras.toLocaleString('es-MX', { minimumFractionDigits: 2 })}`, 140, finalY);
